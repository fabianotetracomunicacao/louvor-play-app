...
DECLARE
    ...
    password_hash text;
BEGIN
    -- 1. LIMPAR TUDO PRIMEIRO (ordem reversa para evitar FK)
    ...
    -- Pequena pausa para garantir que triggers processaram
    PERFORM pg_sleep(0.5);
    
    -- 2. Gerar o hash da senha usando pgcrypto
    password_hash := extensions.crypt(target_password, extensions.gen_salt('bf'));
    
    RAISE NOTICE 'Hash gerado: %', password_hash;
    
    -- 3. Inserir em auth.users com TODOS os campos exatos
    ...
    -- 4. Inserir em auth.identities com estrutura EXATA
    ...
    -- 5. Verificar se profile já existe (pode ter sido criado por trigger)
    ...
    ELSE
        RAISE NOTICE '⚠️ public.profiles já existe (criado por trigger)';
    END IF;
